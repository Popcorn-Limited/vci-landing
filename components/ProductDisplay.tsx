      ...
      >
        <div className="flex-row flex smmd:py-10 py-6 items-start">
          ...
          <div className="flex flex-col justify-start gap-y-4 grow">
            <p className="text-2xl smmd:text-4xl leading-none">{title}</p>
            <p className={`smmd:text-lg text-[#737373] flex ${logoColor === 'black' ? 'md:hidden' : 'md:flex'}`}>{description}</p>
          </div>
          <div className="grow smmd:grow-0 flex flex-row justify-end smmd:items-center items-start h-8 w-8 smmd:w-16 smmd:h-16 smmd:self-center -mt-3 smmd:mt-0">
            {image(logoColor)}
          </div>
        </div>
      </div>
